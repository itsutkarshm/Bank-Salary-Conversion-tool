...
if uploaded_file:
    with st.spinner("⏳ Processing Salary Sheet..."):
        ...
        if missing:
            ...
            # Filter out rows where Employee ID is missing or blank
            df = df[df["Employee"].notna() & (df["Employee"].str.strip() != "")]

            if df.empty:
                st.warning("⚠️ No valid rows with Employee ID found in the file.")
            else:
                # All Headers
                all_headers = [
                    "Client_Code",
                ...
                for _, row in df.iterrows():
                    ...
                    month = row["Month"]
                    cfl = row["CFL"]
                    cfl_clean = "" if pd.isna(cfl) or str(cfl).strip().lower() == "nan" else str(cfl).strip()
                ...
                # === Download Full File ===
                st.markdown("### 📤 Download Full Bank Transfer File")

                full_excel_buffer = BytesIO()
                converted_df.to_excel(full_excel_buffer, index=False, engine="openpyxl")
                full_excel_buffer.seek(0)

                st.download_button(
                    label="📥 Download Full Bank Transfer File",
                    data=full_excel_buffer,
                    file_name="Full_Bank_Transfer_File.xlsx",
                    ...
                    if len(selected_cfls) == 1:
                        ...
                        with zipfile.ZipFile(zip_buffer, "w") as zipf:
                            for cfl in selected_cfls:
                                ...
                                excel_buffer.seek(0)
                                zipf.writestr(
                                    f"{cfl}_Bank_Transfer_File.xlsx",
                                    excel_buffer.read(),
                                )
